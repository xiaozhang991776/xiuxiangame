...
const DIFF = {
    rateMult: 0.5,        // 修炼/点击/闭关/离线 修为获取速率 ×0.5（修为更慢）
    breakXiuMult: 2,      // 突破所需修为 ×2（突破更难）
    breakStoneMult: 0.3,  // 突破灵石门槛 ×0.3（大幅降低，更易突破；想调回更难就调大此值）
    tribChanceMult: 0.85, // 天劫硬抗成功率 ×0.85（更难过）
    tribPenaltyMult: 1.8, // 天劫失败惩罚 ×1.8（境界回落/损修为/损寿元更重）
    incomeMult: 0.5       // 战斗/游历/探索/任务/招财丹 灵石·修为收入 ×0.5（资源更紧，叠×3 → 原×1.5）
};
