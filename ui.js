...
const UI = {
    ...
    doReincarnate(mode) {
        ...
        if (mode === 'free') {
            this.showConfirm('确认免费轮回？', '散功重修：境界、<b>战力</b>、装备、寿元将重置归零，重踏仙途从零修炼；但你的气血/攻击永久加成、修炼收益、悟性、劫后余韵等全部保留，凭存留根基更快重登仙途。', doIt);
        } else {
            const cfg = GameConfig.rebirth;
            const have = (p.inventory.material && p.inventory.material[cfg.herbId]) || 0;
    ...
    TUTORIAL_STEPS: [
        ...
        { title: '突破境界', body: '战力攒满，便点「<b>突破</b>」精进境界、增寿元、强根基。每<b>突破大境界</b>会触发<b>天劫</b>（可请护道人化解或硬抗，成功则永久增益修炼与灵石），并<b>觉醒天赋点</b>。※ <b>道祖之后</b>的超脱系（道君→大道共 20 境）突破所需战力暴增 <b>×30</b>，当早做积累。', panel: 'cultivate', target: '#breakBtn' },
        ...
        { title: '道途恒长', body: '「<b>任务</b>」指引方向，「<b>坊市</b>」补给资源，「<b>道友</b>」论道比拼。仙途漫漫，善自珍重，去罢！', panel: 'quest' }
    ],
    tutorialIndex: 0,
